...
public class Widget {
	...
	private static Cache models = new Cache(30);
	...
	public static void load(Archive interfaces, Archive graphics, Font[] fonts) {
		...
		while (buffer.getPosition() < buffer.getPayload().length) {
			...
			Widget2 widget = /*widgets[id] =*/ new Widget2();
			widget.id = id;
			...
			/*
			 * 414 has parentID/mouseOverId here whereas 317 has a "hoverId"
			 */
...
/*below is not in 414*/
			...
			if (widget.group == TYPE_INVENTORY) {
				...
				for (int index = 0; index < 20; index++) {
					...
					if (exists == 1) {
						...
						String name = buffer.readString();//this is an int in 414
			...
			if (widget.group == TYPE_TEXT || widget.group == TYPE_MODEL_LIST) {//this is really different.
			...
			if (widget.group == TYPE_RECTANGLE || widget.group == TYPE_TEXT) {
				...
				name = buffer.readString();//this is an int in 414
				if (graphics != null && name.length() > 0) {
					int index = name.lastIndexOf(",");
					widget.secondarySprite = getSprite(graphics, name.substring(0, index),
							Integer.parseInt(name.substring(index + 1)));
				}
			} else if (widget.group == TYPE_MODEL) {//this is different in 414
				int content = buffer.readUByte();
				if (content != 0) {
					widget.defaultMediaType = 1;
					widget.defaultMedia = (content - 1 << 8) + buffer.readUByte();//this is an short in 414
				}

				content = buffer.readUByte();
				if (content != 0) {
					widget.secondaryMediaType = 1;
					widget.secondaryMedia = (content - 1 << 8) + buffer.readUByte();//this is an short in 414
				}

				content = buffer.readUByte();
				widget.defaultAnimationId = (content != 0) ? (content - 1 << 8) + buffer.readUByte()
						: -1;//this is an short in 414

				content = buffer.readUByte();
				widget.secondaryAnimationId = (content != 0) ? (content - 1 << 8) + buffer.readUByte()
						: -1;//this is an short in 414
	...
	public Model getAnimatedModel(int primaryFrame, int secondaryFrame, boolean secondary) {
		Model model = secondary ? getModel(secondaryMediaType, secondaryMedia)
				: getModel(defaultMediaType, defaultMedia);
		...
		Model animated = new Model(model, true, Frame.isInvalid(primaryFrame) & Frame.isInvalid(secondaryFrame),
				false);
		if (primaryFrame != -1 || secondaryFrame != -1) {
			animated.prepareSkeleton();
		}
